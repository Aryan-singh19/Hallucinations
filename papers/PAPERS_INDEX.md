...
This repository contains a curated collection of scholarly research papers, technical monographs, and open-access PDFs specifically investigating **hallucination pathologies in Automatic Speech Recognition (ASR), Speech Foundation Models (OpenAI Whisper, SeamlessM4T, AudioPaLM), and Speech Decoders**.

Every single paper is 100% focused on **speech recognition / speech-to-text** transcription errors, acoustic-linguistic alignment failure, cross-attention entropy, and acoustic grounding.

---

## 📑 Research Organization

The library is organized into three core tracks:
1. **`01_definitions/`**: Definitions, taxonomies, non-speech audio triggers, distribution shifts, and ASR failure modes.
2. **`02_quantification/`**: Evaluation metrics, the breakdown of Word Error Rate (WER), cross-attention entropy, spectral dynamics, monotonicity indices, and human-annotated ASR benchmarks.
3. **`03_grounding/`**: Grounding techniques, hybrid CTC-AED anchoring, monotonic attention windowing, Sparse Autoencoder (SAE) latent steering, null-token grounding, and contrastive layer decoding.
...
| **Rethinking Entropy Allocation in LLM-based ASR: Understanding Dynamics between Speech Encoders and LLMs** | Speech AI Research Lab | [arXiv:2604.08003](https://arxiv.org/abs/2604.08003) | `rethinking_entropy_allocation_llm_based_asr_2604.08003.pdf` | Quantifies information-theoretic entropy trade-offs between speech acoustic encoders and autoregressive text decoders. |
...
| **The Null Token Knows: Reducing Message-Free Hallucination in ASR and NMT** | Speech & MT Processing Team | [arXiv:2608.15940](https://arxiv.org/abs/2608.15940) | `the_null_token_knows_reducing_hallucination_asr_2608.15940.pdf` | Explicitly models null/silent acoustic tokens to suppress message-free ungrounded speech emissions in ASR systems. |
